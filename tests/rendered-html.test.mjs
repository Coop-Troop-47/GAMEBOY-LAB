...
test("keeps every hardware input and new option accessible", async () => {
  ...
  assert.match(css, /\.screen-frame\s*\{[^}]*border:\s*calc\(var\(--lcd-bezel-width,\s*8px\) \* 2 \/ 3\) solid #383b43[^}]*border-radius:\s*3px/s);
  assert.doesNotMatch(css, /\.screen-frame\s*\{[^}]*outline:/s);
  assert.match(source, /className="screen-inner"/);
  assert.match(css, /\.dmg \.screen-inner\s*\{[^}]*border:\s*calc\(var\(--lcd-bezel-width,\s*8px\) \/ 3\) solid #121719/s);
  ...
  assert.match(css, /\.screen-frame\s*\{[^}]*container-type:\s*inline-size[^}]*isolation:\s*isolate[^}]*overflow:\s*clip[^}]*border:\s*calc\(var\(--lcd-bezel-width,\s*8px\) \* 2 \/ 3\) solid #383b43/s);
...
test("renders reference-matched DMG and supplied-purple GBC shells", async () => {
  ...
  assert.match(css, /\.screen-only \.cgb \.screen-frame\s*\{[^}]*border-radius:\s*calc\(var\(--lcd-bezel-width,\s*8px\) \* 1\.25\)/s);
