...
test("keeps every hardware input and new option accessible", async () => {
  ...
  assert.match(source, /pending\.targetStyle = Object\.fromEntries\([\s\S]*pending\.bezelAnimation = frame\.animate\([\s\S]*\.\.\.pending\.targetStyle[\s\S]*easing:\s*"linear"/s);
  ...
  assert.match(source, /frame\.style\.setProperty\(property, pending\.targetStyle\[key\]\)[\s\S]*setViewModeTransition\(""\)[\s\S]*requestAnimationFrame/s);
  ...
  assert.match(source, /frame\.style\.removeProperty\(property\)[\s\S]*const targetStyle = window\.getComputedStyle\(frame\)[\s\S]*SCREEN_FRAME_ANIMATED_STYLES\.map\(\(\[key\]\) => \[key, targetStyle\[key\]\]\)/s);
